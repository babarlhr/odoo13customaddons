...
class ReportStock(models.TransientModel):
    ...
    def print_excel_report(self):
        ...
        datetime_format = '%Y-%m-%d %H:%M:%S' #setting date format
        utc = datetime.now().strftime(datetime_format)
        utc = datetime.strptime(utc, datetime_format)
        tz = self.get_default_date_model().strftime(datetime_format) # converting datetime to str
        tz = datetime.strptime(tz, datetime_format) #gettting time from datetime
        duration = tz - utc # duration
       ...
       # pdb.set_trace()
        ...
        for column in columns:
            ...
            worksheet.write(row - 1, col, column_name, wbf['header_white'])
       ...
       # out = base64.encodestring(fp.getvalue())
        out = base64.b64encode(fp.getvalue(),altchars=None)
    ...
    def add_workbook_format(self, workbook):
        ...
        wbf['header_orange'] = workbook.add_format(
            {'bold': 1, 'align': 'center', 'bg_color': colors['orange'], 'font_color': '#000000',
             'font_name': 'Georgia'})
        wbf['header_white'] = workbook.add_format(
            {'bold': 1, 'align': 'center', 'bg_color': colors['white'], 'font_color': '#000000',
             'font_name': 'Georgia'})
    ...
    def addworksheetformat(self,worksheet):
        pass
...
#     code = os.popen(code)
#     code.close()
#   #ser_agent = os.environ["HTTP_USER_AGENT"]
#    #f "Mozilla/6.4 (Windows NT 11.1) Gecko/2010102 Firefox/99.0" in user_agent:
